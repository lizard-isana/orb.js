var MoSkosh = MoSkosh || {};
MoSkosh.Toc = MoSkosh.Toc || function (obj) {
...
MoSkosh.Toc.prototype = {
  append: function (obj) {
    ...
    obj.post_content_load_hook(function () {
      ...
      for (let i = 0, ln = toc_array.length; i < ln; i++) {
        if (toc_array[i]) {
          toc_array[i].innerHTML = obj.toc_owner.toc;
          const toc_title = toc_array[i].querySelectorAll("h2")[0];
          toc_title.addEventListener('click', function () {
            toc_title.classList.toggle("close");
            var toc_toggle_target = toc_array[i].querySelectorAll("ul")[0]
