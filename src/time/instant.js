...
function requireFinite(value, label) {
  if (!Number.isFinite(value)) throw new TypeError(`Instant: ${label} must be finite`);
  return value;
}
...
function parseOptions(options) {
  ...
  if (Math.abs(dut1) > 0.9) {
    throw new RangeError('Instant: dut1 must be within -0.9 and +0.9 seconds');
  }
  return { dut1 };
}
...
export class Instant {
  ...
  static from(value, options) {
    ...
    if (value instanceof Date) return Instant.fromDate(value, options);
    if (typeof value === 'string') return Instant.fromISO(value, options);
    if (typeof value === 'number') return Instant.fromUnixMs(value, options);
    throw new TypeError('Instant.from: expected an Instant, Date, ISO string, or Unix milliseconds');
  }

  static fromDate(date, options) {
    if (!(date instanceof Date)) throw new TypeError('Instant.fromDate: expected a Date');
    return Instant.fromUnixMs(date.getTime(), options);
  }

  static fromISO(iso, options) {
    if (typeof iso !== 'string' || iso.trim() === '') {
      throw new TypeError('Instant.fromISO: expected a non-empty ISO string');
    }
    let value = iso.trim();
    if (/T/.test(value) && !/(?:Z|[+-]\d\d(?::?\d\d)?)$/i.test(value)) value += 'Z';
    return Instant.fromUnixMs(Date.parse(value), options);
  }
  ...
  static fromUnixMs(ms, options) {
    ...
    return new Instant(jd1, jd2, ms, dut1);
  }

  static fromJD(jd, scale = 'tt', options) {
    return Instant.fromJD2(jd, 0, scale, options);
  }

  static fromJD2(jd1, jd2, scale = 'tt', options) {
    requireFinite(jd1, 'jd1');
    requireFinite(jd2, 'jd2');
    if (!VALID_SCALES.has(scale)) throw new RangeError(`Instant.fromJD: unknown scale '${scale}'`);
    const { dut1 } = parseOptions(options);
    let utcMs;
    if (scale === 'utc') {
    ...
    if (scale === 'tt') {
      ...
      return new Instant(tt1, tt2, utcMs, dut1);
    }
    return Instant.fromUnixMs(utcMs, { dut1 });
  }
  ...
  jd2parts(scale = 'tt') {
    if (!VALID_SCALES.has(scale)) throw new RangeError(`Instant.jd: unknown scale '${scale}'`);
  ...
  addSeconds(seconds) {
    ...
    return Instant.fromUnixMs(this.utcMs + seconds * 1000, { dut1: this.dut1 });
  }
  ...
  differenceSeconds(other) {
    const instant = Instant.from(other);
